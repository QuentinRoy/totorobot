...
- [x] Spacing is load-bearing: exactly one spelling, no trimming or
...
- [x] A malformed key poisons its own value type so the error lands on the row,
      not on the table — an intersected missing property reports at the object
      level and is not acceptable
- [x] The vocabulary uses constrained defaults, so widening falls out of the
      constraint with no conditional expressing "nothing declared"
- [x] `NoInfer` on `initial`, in a plain position and **not** wrapped in a
      ...
      line.
- [x] Narrowing the current state narrows its data, with no nullable padding
- [x] `start` and `send` arity follow the initial state's data and the input's
      payload respectively
- [x] Declining is returnable for every target shape including a data-free one,
      without excusing a wrong-shaped return on the same row
- [x] The derived vocabulary and reverse-index types resolve over a machine type
- [x] Unknown names in an observation pattern are rejected; there is no wildcard
      in the label position and a bare key is not a legal pattern
- [x] Declaring one vocabulary map and omitting the other checks that half and
      widens the other
- [x] `pnpm test` and `pnpm typecheck` are both green
