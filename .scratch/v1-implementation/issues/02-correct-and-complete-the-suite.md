...
- [ ] The pattern tests observe through a **host**, not a definition. They
...
- [ ] The type-level degradation tripwires move out of `explorations/` and into
      the suite as a type test; the exploration file is deleted. Tripwires belong
      in tests.
- [ ] `initial` gains the negative coverage the suite has none of: rejected when
...
- [ ] No test asserts on internals — every assertion goes through the public
