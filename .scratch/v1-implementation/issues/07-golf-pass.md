...
- [x] The alternatives the spec deliberately left open are tried and measured —
      listener storage (copy-on-write at registration versus a snapshot per
      dispatch), and getters versus assigned properties
- [x] Terser options are tuned against real numbers rather than assumed
- [x] Measured deltas are recorded in source comments beside the shape they
      justify, including the alternatives that were rejected and by how much
- [x] The source still reads normally — real identifier names, no dense
      ...
      number.
- [x] `pnpm test` and `pnpm test:dist` stay green throughout
- [x] The prototype numbers in the spec are superseded by real ones from the
