// Measures `actions` key completion against the twenty-state, forty-four-row
// acceptance machine (docs/acceptance-cases.md, case 4; the same rows as
// tests/scale.test-d.ts), imported through the real library rather than a
// candidate prototype. `scripts/measure-completions.mjs` probes this file the
// same way it probes `observe-machine/playground.ts` (#116): insert a partial
// key after the anchor and ask for completions.
//
// `A` defaults to `never` (#117), so with the `actions` block empty the
// contextual type for a new key has no known members and nothing is offered
// — this fixture is what turns that claim into a number, before and after
// any change to `Actions`'s completion surface.
...
// <- anchor: the script inserts a new key right after `actions: {`
