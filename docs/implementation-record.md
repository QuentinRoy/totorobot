...
falls back to `string`, `Key<I, S>` collapses, and every row — well formed or not —
is rejected as `not a transition: '…'`. The collapse does not stay in the table
...
already-declared vocabulary — the shape `actions` would take — contributes nothing
to inference, is therefore checked after `S` resolves, and accepts a wrapper with
...
Where a wrapper does work — a single-tier vocabulary, handed to it up front — the
