...
> entry, so a consumer can name them. Exporting a machine now survives a
> plain `tsc --declaration` build, closing TS4023. TS2742 is untouched — it
> is about package layout, not this fix.
>
> TS9010 stays open by design: `--isolatedDeclarations` refuses to infer an
> exported value's type under any circumstance, and a machine's type is
> always inferred. The explicit-model path [note
> 06](research/06-typescript-type-engineering.md) called for turns out to be
> `Machine`/`Host` themselves, now that they can be named. A consumer under
> that flag writes the machine's type by hand instead of exporting it
