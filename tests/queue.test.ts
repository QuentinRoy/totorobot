...
describe('commit ordering', () => {
	test('a send from inside an observer does not take effect before the remaining observers for the current transition have run', () => {
		...
		doc.observe('* -> *', () => {
			...
			if (!queued) {
				queued = true
				doc.send('toggle') // must not take effect before the observer below runs
			}
		})
		doc.observe('* -> *', () => {
			log('second')
	...
	test('several queued sends drain first-in-first-out', () => {
		const counter = machine({
			...
			states: type<{ ready: { order: number[] } }>(),
			transitions: {
				'ready -push> ready': ({ fromData, inputData }) => ({
					order: [...fromData.order, inputData.value],
				}),
			},
		})
	...
	test('an observer that throws propagates out of send; later observers and the queue are abandoned, but the transition stays committed', () => {
		const doc = toggle.start()
		const log = vi.fn()
		let queued = false
		let thrown = false
	...
	test('the drain flag resets after a throw, so a send from inside an observer still queues and drains afterwards', () => {
		const doc = toggle.start()
		...
		doc.observe('* -> *', () => {
			...
			if (!queued) {
				queued = true
				doc.send('toggle') // sent from inside an observer, not top-level
			}
		})
	...
	test('an observer is never re-entered while an earlier call for it is still running', () => {
		const doc = toggle.start()
		let active = 0
		let reentered = false
		let queued = false
		...
		doc.observe('* -> *', () => {
			...
			if (!queued) {
				queued = true
				doc.send('toggle') // would re-enter this same observer if nested rather than queued
			}
			active--
		})
	...
	test('a send from inside an observer mid-chain waits for the whole chain to settle, not just the current hop', () => {
		const relay = machine({
			initial: 'a',
	...
	test('an immediate self-loop that rewrites its data and eventually skips terminates normally, well inside the budget', () => {
		const counter = machine({
			initial: 'idle',
			inputs: type<{ go: undefined }>(),
			states: type<{ idle: undefined; counting: { count: number } }>(),
			transitions: {
				'idle -go> counting': () => ({ count: 0 }),
				'counting -> counting': ({ fromData, skip }) =>
					fromData.count < 5 ? { count: fromData.count + 1 } : skip(),
			},
		})
		...
		expect(doc.current).toEqual({ name: 'counting', data: { count: 5 } })
	})
...
describe('commit ordering across hosts', () => {
	test("a send from one host into another queues rather than nesting: the second host runs only after the first host's remaining observers for the current transition", () => {
	...
	test('the whole thing drains before the outermost send returns, synchronously, across hosts', () => {
		...
		expect(hostB.current.name).toBe('on')
	})

	test('several sends from observers across hosts drain first-in-first-out', () => {
		const counter = machine({
			initial: 'ready',
			inputs: type<{ push: { value: number } }>(),
			states: type<{ ready: { order: number[] } }>(),
		...
		hostB.observe('* -> *', (e) => log(`B${e.toData.order.at(-1)}`))
	...
	test('a send from inside an observer into another host, issued mid-chain, drains only once the chain has settled, never mid-hop', () => {
		const relay = machine({
			initial: 'a',
			inputs: type<{ go: undefined }>(),
			states: type<{ a: undefined; b: undefined; c: undefined }>(),
		...
		hostA.observe('* -> *', (e) => log(`A -> ${e.to}`))
		hostB.observe('* -> *', () => log('B toggled'))

		hostA.send('go')

		expect(hostA.current.name).toBe('c')
		expect(log).toHaveBeenCalledTimes(4)
	...
	test('an observer throwing in a second host unwinds out of the send that started the chain, discarding everything still queued across both hosts, and both stay usable afterwards', () => {
		const hostA = toggle.start()
		const hostB = toggle.start()
		const log = vi.fn()
		let queued = false
		...
		hostA.send('toggle')
		hostB.send('toggle')
		expect(hostA.current.name).toBe('off')
		expect(hostB.current.name).toBe('off')
	})

	test('a runaway immediate chain in one host discards queued sends in another host the same way a throwing observer does', () => {
		...
		expect(hostB.current.name).toBe('off') // never drained

		// both hosts still work afterwards
		hostA.send('stop')
		hostB.send('toggle')
		expect(hostA.current.name).toBe('idle')
		expect(hostB.current.name).toBe('on')
	})
...
// the host being started can issue one — it has no observers yet — so these go
...
describe('commit ordering while `start` settles', () => {
	test('a send issued while the initial chain settles queues rather than nesting, and drains after the chain has settled', () => {
		...
		const log = vi.fn()
		sink.observe('* -> *', (e) => log(`sink observer: -> ${e.to}`))
		...
		expect(started.current.name).toBe('c')
		expect(log).toHaveBeenCalledTimes(4)
		...
		expect(log).toHaveBeenNthCalledWith(4, 'sink observer: -> on')
		// drained before `start` returned
		expect(sink.current.name).toBe('on')
	})
	...
	test('`start` called from inside a dispatch settles inline but leaves the queue to the outer drain', () => {
		...
		driver.observe('* -> *', () => {
			...
			expect(late.start().current.name).toBe('b')
			log('driver observer done')
		})

		driver.send('toggle')
		expect(log).toHaveBeenCalledTimes(3)
		expect(log).toHaveBeenNthCalledWith(1, 'sink right after send: off')
		expect(log).toHaveBeenNthCalledWith(2, 'driver observer done')
		expect(log).toHaveBeenNthCalledWith(3, 'sink observer: -> on')
	})
	...
	test('a runaway initial chain discards what its own hops queued, and every host stays usable', () => {
		...
		expect(sink.current.name).toBe('off') // never drained
...
describe('actions fired by `start()`', () => {
	...
	test('a self-send from the initial residency drains before `start` returns, so the host it hands back already reflects it — with no observer able to have missed the hop, since none could yet exist', () => {
