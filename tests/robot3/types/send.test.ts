...
test('types machine with multiple transitions from one state', () => {
	...
	type ObjectParams = Extract<EventParam, { type: string }>
	expectTypeOf<ObjectParams['type']>().toEqualTypeOf<
...
test('types nested machine', () => {
	const stopwalk = createMachine({
		walk: state(transition('startBlinking', 'blink')),
		blink: state(transition('finishBlinking', 'dontWalk')),
		dontWalk: state(),
	})
	...
	const stoplight = createMachine({
		...
		red: invoke(stopwalk, transition('done', 'green')),
	})
	...
	type EventParam = Params[0]
	type StringParams = Extract<EventParam, string>
	expectTypeOf<StringParams>().toEqualTypeOf<
		'next' | 'startBlinking' | 'finishBlinking'
	>()
