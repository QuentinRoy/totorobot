 ...
 * annotation. `keep` and `repeat` name the current state.
 */
export type Edge<
	S extends AnyStates,
	I extends AnyInputs,
...
> =
	...
	// `object &` is load-bearing. Every property below is optional, so without
	// it a bare string is structurally assignable to this member and EVERY
	// invalid target name is silently accepted. Until this was added the only
	// thing catching them was that `String.prototype.repeat` happens to collide
	// with the reserved `repeat` key -- an accident, not a design.
	| (object & {
