...
| [`wrapper-inference.ts`](wrapper-inference.ts)             | a table value as a wrapper call (`transition(fn)`) rather than a bare function or a record                                                                        | **Depends on the block, not the value.** In `transitions` a wrapper fails twice over: the call is checked before `machine`'s defaulted `I`/`S` exist, so every row reads as malformed ([I24](../docs/implementation-record.md#i24)), and where a wrapper does work the row key comes back by alias identity rather than structurally ([I25](../docs/implementation-record.md#i25)). In a block keyed off the declared vocabulary, which is `actions`' shape, the same wrapper works, provided something hands it that vocabulary; recovering it from context leaves the payload `never` while the key stays constrained, which reads as success to a probe that only checks bad keys ([I26](../docs/implementation-record.md#i26)). The two encouraging results are kept beside the failures, since a probe meets them first. |
