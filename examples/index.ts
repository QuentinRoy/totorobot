...
traffic.observe('* -> *', (e) => {
	console.log(`  ${e.from} -${e.input}> ${e.to}`, e.toData)
})
...
auth.observe('* -> *', (e) => console.log(`  -> ${e.to}`, e.toData))
...
search.observe('* -> *', (e) => console.log(`  -> ${e.to}`))
...
search.observe('typing', {
	run: (e) => console.log(`  [ping] composing, from "${e.toData.query}"`),
	restart: false,
})
search.observe('loading -results> results', (e) =>
	console.log(
		`  [edge] ${e.toData.items.length} hit(s) for "${e.toData.query}"`,
	),
)
