...
actions: {
	...
	'draft -submit> review': ({ transition }) => track('submitted'),
}
```
...
Actions run in block declaration order, ahead of every listener, and a throwing
action propagates the same way a throwing listener does.
